...
video1,fps = read_video(video1) # frames, fps
video1 = video1/255.0
...
print('Conteo por sonido: ', outputs11)

# Conteo por vista
outputs = get_visual_count(video1, sample_rate, model, tensor)
print('Conteo por vista: ', outputs)

conf = get_conf(spectrogram_ori, outputs11, sample_rate, video1, audio_model, model, conf_model)
print(conf)

# Conteo por visión y sonido
outputs = conf*outputs + (1-conf)*outputs11
outputs = np.round(outputs)
print('Conteo final: ', outputs)

# Calculo del error entre la predicción
groundtruth=8 # Número real de repeticiones del vídeo
mae_err = abs(outputs-groundtruth) / groundtruth # Cálculo del error
